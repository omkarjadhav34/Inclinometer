...
void mpu6050_getData(XIic* i2c, MPU6050* mpu6050, int *angle_actual, int axis)
{
	...
	//XIic_Recv(i2c->BaseAddress, (axis == SET_AXIS_X) ? GYRO_XOUT_2 :
	//								(axis == SET_AXIS_Y) ? GYRO_YOUT_2 : GYRO_ZOUT_2, (u8 *)&iBuf, 1, XIIC_STOP);

	*angle_actual = ((u8)iBuf) * 10;

	// get the higher nibble data of a particular axis
	//XIic_Recv(i2c->BaseAddress, (axis == SET_AXIS_X) ? GYRO_XOUT_1 :
	//								(axis == SET_AXIS_Y) ? GYRO_YOUT_1 : GYRO_ZOUT_1, (u8 *)&iBuf, 1, XIIC_STOP);
